...
<body>
    ...
    <?php
        ...
        $sql = 'create table if not exists tb_mission5'
                .'('
                .'id INT AUTO_INCREMENT PRIMARY KEY,'   //自動で登録される番号
            ...
            if(isset($_POST['name']) && $_POST["name"] !== ""){
                ...
                if(isset($_POST['comment']) && $_POST['comment'] !== ""){
                    ...
                    if(isset($_POST['hidden']) && $_POST['hidden'] !== ""){
                        //echo "1<br>";
                        $sql = 'update tb_mission5 set name = :name, comment = :comment, date = :date, password = :password where id = :id';
                    ...
                    //テーブルtb_mission5に新規登録($_POST['hidden']に値がないとき)
                        ...
                        $sql = $pdo -> prepare('insert into tb_mission5(name, comment, date, password) values(:name, :comment, :date, :password);');
        ...
        elseif (isset($_POST['delete']) && $_POST['delete'] === "削除") {
            ...
            if(isset($_POST['d-num']) && $_POST['d-num'] !== ""){
                ...
                if(isset($_POST['d-pass']) && $_POST['d-pass'] !== ""){
                    ...
                    $sql = 'select * from tb_mission5 where id = :id && password = :password;';
                    $stmt = $pdo -> prepare($sql);
                    $stmt -> bindParam(':id', $id, PDO::PARAM_INT);
                    $stmt -> bindParam(':password', $pass, PDO::PARAM_STR);
                    $id = $_POST['d-num'];
                    ...
                    if($i === 0){
                        ...
                        $sql = 'delete from tb_mission5 where id = :id && password = :password;';
                        $stmt = $pdo -> prepare($sql);
                        $stmt -> bindParam(':id', $id, PDO::PARAM_INT);
                        $stmt -> bindParam(':password', $pass, PDO::PARAM_STR);
                        $id = $_POST['d-num'];
    ...
    ※新規投稿した時に最終行番号と連番にならないときは、最終行が削除されたためです。<br>
